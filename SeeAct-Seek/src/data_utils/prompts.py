...
seeact_online_sys_prompt = '''Imagine that you are imitating humans doing web navigation to aggregate information for a task step by step. 
...
You need to decide on the first following action to take. You can click on an element with the mouse, select an option, type text or press Enter with the keyboard. (For your understanding, they are like the click(), select_option() type() and keyboard.press('Enter') functions in playwright respectively) One next step means one operation within the four. Unlike humans, for typing (e.g., in text areas, text boxes) and selecting (e.g., from dropdown menus or <select> elements), you should try directly typing the input or selecting the choice, bypassing the need for an initial click. Further on the current webpage, you can decide whether to aggregate information or not (the action "AGGREGATE INFORMATION" will trigger a web crawling operation to aggregate factual information from the page). You can go back whenever you find it necessary to visit other websites. 
Once you decide to aggregate information, another agent aggregates necessary information for the task from that website and provides feedback on what information should be aggregtated next. You should navigate with that information.
You should not attempt to create accounts, log in or do the final submission. Terminate when you deem the task complete or if it requires potentially harmful actions.'''
...
To be successful, it is important to follow the following rules: 
1. You should only issue a valid action given the current observation. 
2. if the current webpage has relevant information for the task, trigger AGGREGATE INFORMATION. Even if the information is readily available from the screenshot, you have to trigger the AGGREGATE INFORMATION function alongside producing the reasons of triggering it
3. AGGREGATE INFORMTION is to be used when you think there are factual information that might be useful in this current page
4. You should only issue one action at a time
5. in order to search for a query, if the previous cation was to type in the query, it is important to press enter next. Also make sure to mention the query explicitly alongside the action
6. DO NOT trigger AGGREGATE INFORMATION from google search page
7. for AGGREGATE INFORMATION visit popular resources like wikipedia, tripadvisor etc.
8. For handling the select dropdown elements on the webpage, it's not necessary for you to provide completely accurate options right now. The full list of options for these elements will be supplied later.
9. to look for new information, you can go back to google.com and search for it
10. scroll is strictly not an allowed action
11. if you took the same action multiple times in a sequence its probably due to a reasoning error, and you should replan the execution
'''

seeact_online_action_format = "ACTION: Choose an action from {CLICK, SELECT, TYPE, PRESS ENTER, GO BACK, TERMINATE, NONE, AGGREGATE INFORMATION}."

seeact_online_value_format = "VALUE: Provide additional input based on ACTION.\n\nThe VALUE means:\nIf ACTION == TYPE, specify the " \
               "text to be typed.\nIf ACTION == SELECT, indicate the option to be chosen. Revise the selection value to align with the available options within the element.\nIf ACTION == CLICK, PRESS ENTER, GO BACK, TERMINATE, AGGREGATE INFORMATION or NONE, " \
               "write \"None\"."
...
If the target action requires interacting with an element, choose the element by doing the following multi-choice question answering. 
If the target action is going back, terminating, aggregating information or doing nothing, you should select the element to be the last one ( i.e. none ).
...
ELEMENT: The uppercase letter of your choice. (for PRESS ENTER, AGGREGATE INFORMATION or GO BACK, select the last option)""",

...
def get_search_prompt(question, passages, instruction):
    prompt = """Based the provided context, answer the following question. Output only your answer. Never refuse to answer. If the answer is a list, output one on each line. 
    The answer should strictly come from the context, dont add your own knowledge to it.

    Formatting instruction for final answer: {instruction}
    ...
    return prompt.format(context=context, question=question, instruction=instruction)


def format_final_answer(question, passages, instruction):
    llm = ChatOpenAI(model="gpt-4o", max_tokens=4000, temperature=0)
    messages = [
                {"role": "user", "content": get_search_prompt(question, passages, instruction)}
            ]
    ...
    if session_control.context:
        ...
        try:
            ...
            if session_control.context.pages:
                ...
                try:
                    await session_control.active_page.goto("https://www.bing.com/", wait_until="load")
...
async def main(config, base_dir) -> None:
    ...
    generation_model = OpenaiEngine(**openai_config, ) # GPT-4V

    # Initialize text-only OpenAI model (GPT-4)    
    openai_config_text = config["openai_text"]
    # generation_model_text = OpenaiEngine_Text(**openai_config_text,)
    # print("Text-only OpenAI model initialized: ", generation_model_text.model)
    infoseeker_config = config["infoseeker"]
    print("Infoseeker config: ", infoseeker_config)
    # infoseeker = None
    ...
    for single_query_task in query_tasks:
        total_extractor_cost = 0
        infoseeker = InfoSeeker(infoseeker_config["type"], openai_config_text)

        confirmed_task = single_query_task["confirmed_task"]
        instruction = single_query_task["instruction"]
        ...
        async with async_playwright() as playwright:
            ...
            while not complete_flag and infoseeker.counter<6:
                print(infoseeker.counter)
                ...
                logger.info('-' * 10)
                print(session_control.active_page.url)
                ...
                ## check page info
                # Extracting text from all <p> elements
                ...
                if len(elements) == 0:
                    ...
                    final_json = {"confirmed_task": confirmed_task, "website": confirmed_website,
                                  ...
                                  "exit_by": "No elements",
                                  "final_answer": format_final_answer(confirmed_task, infoseeker.aggregated_info, instruction),
                                  "extractor_cost": total_extractor_cost,
                                  "aggregator_cost": infoseeker.aggregator_cost
                                  }
                    if infoseeker is not None:
                        final_json["aggregate_info"] = infoseeker.aggregated_info
                ...
                for multichoice_i in range(0, num_choices, step_length):
                    ...
                    # add infoseeking status to the navigation prompt
                    if infoseeker is None:
                        task_status = confirmed_task
                    else:
                        current_feedback = infoseeker.terminate_decisions[-1]["content"]["explanation"]
                        logger.info("feedback")
                        logger.info(current_feedback)
                        task_status = f"{confirmed_task}\n\nCurrent Feedback: {current_feedback}"
                    
                    # Format prompts for LLM inference
                    prompt = generate_prompt(task=task_status, previous=taken_actions, choices=choices,
                    ...
                    ## TODO -> currently seems inorganic, shou
                    if pred_action in ["GO BACK", "AGGREGATE INFORMATION"]:
                        print("Model prediction for element was wrong - {}".format(element_id))
                        element_id = len(candidate_ids)
                        print("Model prediction updated - {}".format(element_id))
                    if (0 <= element_id < len(candidate_ids) and pred_action.strip() in ["CLICK", "SELECT", "TYPE",
                                                                                         "PRESS ENTER", "HOVER",
                                                                                         "TERMINATE", "GO BACK", "AGGREGATE INFORMATION"]):
                        ...
                        got_one_answer = True
                        break
                    elif pred_action.strip() in ["PRESS ENTER", "TERMINATE", "GO BACK", "AGGREGATE INFORMATION"]:
                ...
                try:
                    ...
                    try:
                        if target_element == []:
                            ...
                            if not target_element in ["PRESS ENTER", "TERMINATE", "GO BACK", "AGGREGATE INFORMATION"]:
                        ...
                        if selector:
                            ...
                            logger.info("Try performing a PRESS ENTER")
                            await session_control.active_page.keyboard.press('Enter')
                        elif target_element == "GO BACK":
                            try:
                                logger.info("Try performing a GO BACK")
                                await session_control.active_page.go_back()
                            except Exception as e:
                                logger.info("go back did not work")
                                logger.info(e)
                        elif target_action == "AGGREGATE INFORMATION":
                            logger.info("Aggregating information")
                            if infoseeker is not None:
                                terminate_info_seeking = False
                                paragraphs_data, extractor_cost = await extract_vllm(session_control=session_control, task=confirmed_task, search_motivation=output0) ## TODO
                                total_extractor_cost += extractor_cost
                                # paragraphs_data = await get_text_paragraphs_with_playwright(session_control.active_page, 'span')
                                # paragraphs_data += await get_text_paragraphs_with_playwright(session_control.active_page, 'p')
                                logger.info(f"# paragraphs: {len(paragraphs_data)}")
                                if len(paragraphs_data) == 0:
                                    logger.info("No paragraphs found on the page.")
                                    infoseeker.aggregated_info.append({"step": len(infoseeker.aggregated_info), "content": infoseeker.aggregated_info[-1]["content"]})
                                    infoseeker.raw_page_info.append({"step": len(infoseeker.raw_page_info), "content": "None"})
                                    infoseeker.terminate_decisions.append({"step": len(infoseeker.terminate_decisions), "content": {"terminate": "no", "explanation": infoseeker.terminate_decisions[-1]["content"]["explanation"]}})

                                    await session_control.active_page.goto("https://www.google.com/", wait_until="load")
                                    ##Sagnik -> if no infoseeking is done, we keep the feedback the same
                                else:
                                    paragraphs_text = "\n".join([p for p in paragraphs_data])
                                    paragraphs_text = paragraphs_text[:128000] # limit the length of the text
                                    infoseeker.run(task_query=confirmed_task, new_info=paragraphs_data, search_motivation=output0)  
                                    
                                    infoseeker.raw_page_info.append({"step": len(infoseeker.raw_page_info), "content": paragraphs_text}) ##TODO: sagnik -> right now its just appended everything. and this should happen 
                                    # within the class infoseeker
                                    terminate_yon = infoseeker.terminate_decisions[-1]['content']['terminate']
                                    terminate_info_seeking = True if terminate_yon.lower() == "yes" else False

                                    await session_control.active_page.goto("https://www.google.com/", wait_until="load")
                                
                                # save intermediate information seeking results
                                with open(os.path.join(main_result_path, 'info_seeking_results.json'), 'w', encoding='utf-8') as file:
                                    json.dump({
                                        "aggregated_info": infoseeker.aggregated_info, 
                                        "raw_page_info": infoseeker.raw_page_info,
                                        "terminate_decisions": infoseeker.terminate_decisions,
                                        "final_answer": format_final_answer(confirmed_task, infoseeker.aggregated_info, instruction),
                                        "extractor_cost": total_extractor_cost,
                                        "aggregator_cost": infoseeker.aggregator_cost
                                    }, file, indent=4)

                                if terminate_info_seeking:
                                    logger.info("Terminate because information seeking is done.")
                                    
                                    screen_shot_output_path = os.path.join(main_result_path, 'image_inputs',
                                                    f'{time_step}_0_crop.jpg')
                                    try:
                                        await session_control.active_page.screenshot(path=screen_shot_output_path, clip=clip, full_page=True,
                                    ...
                                    final_json = {"confirmed_task": confirmed_task, "website": confirmed_website,
                                                ...
                                                "num_step": len(taken_actions), "action_history": taken_actions,
                                                "exit_by": "No elements",
                                                "aggregate_info": infoseeker.aggregated_info,
                                                "final_answer": format_final_answer(confirmed_task, infoseeker.aggregated_info, instruction),
                                                "extractor_cost": total_extractor_cost,
                                                "aggregator_cost": infoseeker.aggregator_cost
                                                }
                    ...
                    if new_action == "" or fail_to_execute:
                        ...
                        if monitor_signal not in ["pause", "reject"]:
                            no_op_count += 1
                    taken_actions.append("Action taken at step " + str(time_step) +" : " + new_action + "\n\njustification for action: " + output0.replace("\n", " ") + "\n\n")
                    # if target_action == "AGGREGATE INFORMATION":
                    #     taken_actions.append("Additional action taken at step " + str(time_step) +" : " + "Navigate back to google.com" + "\n\njustification for action: " + "nvigating back to google.com homepage to look for further information" + "\n\n")
                    ...
                    if infoseeker is not None:
                        final_json["aggregate_info"] = infoseeker.aggregated_info,
                        final_json["final_answer"] = format_final_answer(confirmed_task, infoseeker.aggregated_info, instruction)

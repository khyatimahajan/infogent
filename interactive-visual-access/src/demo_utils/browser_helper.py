...
async def get_element_data(element, tag_name):
    ...
    if not description:
        return None
    try:
        # Await the bounding_box with a timeout of 2 seconds
        rect = await asyncio.wait_for(element.bounding_box(), timeout=2)
    except asyncio.TimeoutError:
        # Provide a default value if a TimeoutError occurs
        rect = {'x': 0, 'y': 0, 'width': 0, 'height': 0}

    # rect = await  asyncio.wait_for(element.bounding_box(), timeout=2) or {'x': 0, 'y': 0, 'width': 0, 'height': 0}
    ...
    if real_tag_name == 'a':
        ...
        href_value = await element.get_attribute('href', timeout=0)

        if href_value:

            tag_head += " href=" + "\"" + href_value + "\""
...
async def get_text_paragraphs_with_playwright(page, tag):
    # Selector for paragraph elements
    paragraph_selector = tag
    paragraphs = page.locator(paragraph_selector)

    # List to store paragraphs data
    paragraphs_data = []

    # Count the number of paragraph elements found
    paragraph_count = await paragraphs.count()

    for index in range(paragraph_count):
        paragraph_element = paragraphs.nth(index)
        
        # Check if the paragraph is visible
        if await paragraph_element.is_hidden():
            continue
        
        # Use JavaScript to extract clean text, stripping HTML and decoding entities
        text_content = await paragraph_element.evaluate("""(element) => {
            ...
            return decodeHtmlEntities(element.innerText || '');
        }""")
        ...
        # Get bounding box of the paragraph element
        bounding_box = await paragraph_element.bounding_box()
        if bounding_box:
            box_model = [bounding_box['x'], bounding_box['y'], bounding_box['x'] + bounding_box['width'], bounding_box['y'] + bounding_box['height']]
            center_point = ((box_model[0] + box_model[2]) / 2, (box_model[1] + box_model[3]) / 2)
        else:
            # Default values if bounding box is not available
            box_model = [0, 0, 0, 0]
            center_point = (0, 0)
...
def saveconfig(config, save_file):
    ...
    if isinstance(config, dict):
        with open(save_file, 'w') as f:
            ...
            config_without_key["openai_text"]["api_key"] = "Your API key here"
